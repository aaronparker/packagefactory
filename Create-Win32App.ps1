...
<#
    ...
    .NOTES
        ...
        1.0.1 - (2022-12-29) - CL: Errorhandling if Install.ps1 or Uninstall.ps1 is missing source folder
        1.0.2 - (2023-01-01) - CL: Errorhandling check the real msi GUID if it matches the uninstall string of app.json
...
process {
    ...
    foreach ($AppName in $Application) {
        ...
        if ($($appdata.Program.UninstallCommand) -match '{\w{8}-\w{4}-\w{4}-\w{4}-\w{12}}' -and $AppData.PackageInformation.SetupType -eq "MSI") {
