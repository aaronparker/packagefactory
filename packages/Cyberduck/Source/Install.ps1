...
try {
    ...
    $Installer = Get-ChildItem -Path $PWD -Filter "Cyberduck-Installer*.exe" -Recurse -ErrorAction "SilentlyContinue"
    $params = @{
        FilePath     = $Installer.FullName
        ArgumentList = "/quiet /norestart /log `"C:\ProgramData\PackageFactory\logs\Cyberduck.log`""
        NoNewWindow  = $True
        PassThru     = $True
        Wait         = $True
    }
    ...
    throw "Failed to install Cyberduck."
}
finally {
