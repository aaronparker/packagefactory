...
try {
    ...
    $Installer = Get-ChildItem -Path $PWD -Filter "windowsdesktop*.exe" -Recurse -ErrorAction "SilentlyContinue"
    $params = @{
        FilePath     = $Installer.FullName
        ArgumentList = "/install /quiet /norestart /log `"$env:ProgramData\PackageFactory\Logs\Microsoft.NET.log`""
        NoNewWindow  = $True
        PassThru     = $True
        Wait         = $True
    }
