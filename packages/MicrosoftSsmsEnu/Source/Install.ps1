...
try {
    ...
    $Installer = Get-ChildItem -Path $PWD -Filter "SSMS-Setup*.exe" -Recurse -ErrorAction "SilentlyContinue"
    $params = @{
        FilePath     = $Installer.FullName
        ArgumentList = "/quiet /install /log `"C:\ProgramData\PackageFactory\logs\MicrosoftSsms.log`""
        NoNewWindow  = $True
        PassThru     = $True
        Wait         = $True
    }
    ...
    throw "Failed to install Microsoft SQL Server Management Studio."
}
finally {
