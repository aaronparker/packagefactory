...
Try {
    ...
    [String]$appName = 'TSA Drucker Installieren TSAPR-FM'
    [String]$appVersion = ''
    [String]$appArch = 'x64'
    [String]$appLang = 'DE'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        try {
            Write-Log -Message "Starting installation of network printer \\tsaps1\tsaPR-FM" -Source $deployAppScriptFriendlyName
            ...
            $existingPrinter = Get-Printer -Name "\\tsaps1\tsaPR-FM" -ErrorAction SilentlyContinue
            
            if ($existingPrinter) {
                Write-Log -Message "Printer \\tsaps1\tsaPR-FM already exists. Removing existing printer..." -Source $deployAppScriptFriendlyName
                Remove-Printer -Name "\\tsaps1\tsaPR-FM" -ErrorAction SilentlyContinue
            }
            
            # Add the network printer
            Write-Log -Message "Adding network printer \\tsaps1\tsaPR-FM" -Source $deployAppScriptFriendlyName
            Add-Printer -ConnectionName "\\tsaps1\tsaPR-FM" -ErrorAction Stop
            
            # Verify printer was added successfully
            $newPrinter = Get-Printer -Name "\\tsaps1\tsaPR-FM" -ErrorAction SilentlyContinue
            if ($newPrinter) {
                Write-Log -Message "Successfully installed printer \\tsaps1\tsaPR-FM" -Source $deployAppScriptFriendlyName
                
                # Set as default printer (optional - uncomment if needed)
                # Set-Printer -Name "\\tsaps1\tsaPR-FM" -Default
                # Write-Log -Message "Set \\tsaps1\tsaPR-FM as default printer" -Source $deployAppScriptFriendlyName
                
                # Create installation marker
                Set-Content -Path "$printerDataPath\TSAPrinterTSAPR-FM.ps1.tag" -Value "Installed" -Force
            ...
            Write-Log -Message "Failed to install printer \\tsaps1\tsaPR-FM. Error: $($_.Exception.Message)" -Severity 3 -Source $deployAppScriptFriendlyName
            throw $_
        }
        finally {
            Stop-Transcript
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        try {
            Write-Log -Message "Starting uninstallation of printer \\tsaps1\tsaPR-FM" -Source $deployAppScriptFriendlyName
            
            # Check if printer exists
            $existingPrinter = Get-Printer -Name "\\tsaps1\tsaPR-FM" -ErrorAction SilentlyContinue
            
            if ($existingPrinter) {
                Write-Log -Message "Removing printer \\tsaps1\tsaPR-FM" -Source $deployAppScriptFriendlyName
                Remove-Printer -Name "\\tsaps1\tsaPR-FM" -ErrorAction Stop
                Write-Log -Message "Successfully removed printer \\tsaps1\tsaPR-FM" -Source $deployAppScriptFriendlyName
            } else {
                Write-Log -Message "Printer \\tsaps1\tsaPR-FM not found, nothing to remove" -Source $deployAppScriptFriendlyName
            }
            
            # Remove installation marker
            if (Test-Path "$printerDataPath\TSAPrinterTSAPR-FM.ps1.tag") {
                Remove-Item -Path "$printerDataPath\TSAPrinterTSAPR-FM.ps1.tag" -Force
                Write-Log -Message "Removed installation marker" -Source $deployAppScriptFriendlyName
            }
        }
        catch {
            Write-Log -Message "Failed to uninstall printer TSAPR-FM. Error: $($_.Exception.Message)" -Severity 3 -Source $deployAppScriptFriendlyName
            throw $_
        }
        finally {
            Stop-Transcript
