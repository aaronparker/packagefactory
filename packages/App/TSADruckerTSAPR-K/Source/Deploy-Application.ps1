...
Try {
    ...
    [String]$appName = 'TSA Drucker Installieren TSAPR-K'
    [String]$appVersion = ''
    [String]$appArch = 'x64'
    [String]$appLang = 'DE'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        try {
            Write-Log -Message "Starting installation of network printer \\tsaps1\tsaPR-K" -Source $deployAppScriptFriendlyName
            ...
            $existingPrinter = Get-Printer -Name "\\tsaps1\tsaPR-K" -ErrorAction SilentlyContinue
            
            if ($existingPrinter) {
                Write-Log -Message "Printer \\tsaps1\tsaPR-K already exists. Removing existing printer..." -Source $deployAppScriptFriendlyName
                Remove-Printer -Name "\\tsaps1\tsaPR-K" -ErrorAction SilentlyContinue
            }
            
            # Add the network printer
            Write-Log -Message "Adding network printer \\tsaps1\tsaPR-K" -Source $deployAppScriptFriendlyName
            Add-Printer -ConnectionName "\\tsaps1\tsaPR-K" -ErrorAction Stop
            
            # Verify printer was added successfully
            $newPrinter = Get-Printer -Name "\\tsaps1\tsaPR-K" -ErrorAction SilentlyContinue
            if ($newPrinter) {
                Write-Log -Message "Successfully installed printer \\tsaps1\tsaPR-K" -Source $deployAppScriptFriendlyName
                
                # Set as default printer (optional - uncomment if needed)
                # Set-Printer -Name "\\tsaps1\tsaPR-K" -Default
                # Write-Log -Message "Set \\tsaps1\tsaPR-K as default printer" -Source $deployAppScriptFriendlyName
                
                # Create installation marker
                Set-Content -Path "$printerDataPath\TSAPrinterTSAPR-K.ps1.tag" -Value "Installed" -Force
            ...
            Write-Log -Message "Failed to install printer \\tsaps1\tsaPR-K. Error: $($_.Exception.Message)" -Severity 3 -Source $deployAppScriptFriendlyName
            throw $_
        }
        finally {
            Stop-Transcript
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        try {
            Write-Log -Message "Starting uninstallation of printer \\tsaps1\tsaPR-K" -Source $deployAppScriptFriendlyName
            
            # Check if printer exists
            $existingPrinter = Get-Printer -Name "\\tsaps1\tsaPR-K" -ErrorAction SilentlyContinue
            
            if ($existingPrinter) {
                Write-Log -Message "Removing printer \\tsaps1\tsaPR-K" -Source $deployAppScriptFriendlyName
                Remove-Printer -Name "\\tsaps1\tsaPR-K" -ErrorAction Stop
                Write-Log -Message "Successfully removed printer \\tsaps1\tsaPR-K" -Source $deployAppScriptFriendlyName
            } else {
                Write-Log -Message "Printer \\tsaps1\tsaPR-K not found, nothing to remove" -Source $deployAppScriptFriendlyName
            }
            
            # Remove installation marker
            if (Test-Path "$printerDataPath\TSAPrinterTSAPR-K.ps1.tag") {
                Remove-Item -Path "$printerDataPath\TSAPrinterTSAPR-K.ps1.tag" -Force
                Write-Log -Message "Removed installation marker" -Source $deployAppScriptFriendlyName
            }
        }
        catch {
            Write-Log -Message "Failed to uninstall printer TSAPR-K. Error: $($_.Exception.Message)" -Severity 3 -Source $deployAppScriptFriendlyName
            throw $_
        }
        finally {
            Stop-Transcript
