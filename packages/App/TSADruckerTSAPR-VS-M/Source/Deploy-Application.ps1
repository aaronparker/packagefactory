...
Try {
    ...
    [String]$appName = 'TSA Drucker Installieren TSAPR-VS-M'
    [String]$appVersion = ''
    [String]$appArch = 'x64'
    [String]$appLang = 'DE'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        try {
            Write-Log -Message "Starting installation of network printer \\tsaps1\tsaPR-VS-M" -Source $deployAppScriptFriendlyName
            ...
            if ($existingPrinter) {
                Write-Log -Message "Printer \\tsaps1\tsaPR-VS-M already exists. Removing existing printer..." -Source $deployAppScriptFriendlyName
                Remove-Printer -Name "\\tsaps1\tsaPR-VS-M" -ErrorAction SilentlyContinue
            }
            
            # Add the network printer
            Write-Log -Message "Adding network printer \\tsaps1\tsaPR-VS-M" -Source $deployAppScriptFriendlyName
            ...
            if ($newPrinter) {
                Write-Log -Message "Successfully installed printer \\tsaps1\tsaPR-VS-M" -Source $deployAppScriptFriendlyName
                
                # Set as default printer (optional - uncomment if needed)
                # Set-Printer -Name "\\tsaps1\tsaPR-VS-M" -Default
                # Write-Log -Message "Set \\tsaps1\tsaPR-VS-M as default printer" -Source $deployAppScriptFriendlyName
            ...
            Write-Log -Message "Failed to install printer \\tsaps1\tsaPR-VS-M. Error: $($_.Exception.Message)" -Severity 3 -Source $deployAppScriptFriendlyName
            throw $_
        }
        finally {
            Stop-Transcript
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        try {
            Write-Log -Message "Starting uninstallation of printer \\tsaps1\tsaPR-VS-M" -Source $deployAppScriptFriendlyName
            
            # Check if printer exists
            $existingPrinter = Get-Printer -Name "\\tsaps1\tsaPR-VS-M" -ErrorAction SilentlyContinue
            
            if ($existingPrinter) {
                Write-Log -Message "Removing printer \\tsaps1\tsaPR-VS-M" -Source $deployAppScriptFriendlyName
                Remove-Printer -Name "\\tsaps1\tsaPR-VS-M" -ErrorAction Stop
                Write-Log -Message "Successfully removed printer \\tsaps1\tsaPR-VS-M" -Source $deployAppScriptFriendlyName
            } else {
                Write-Log -Message "Printer \\tsaps1\tsaPR-VS-M not found, nothing to remove" -Source $deployAppScriptFriendlyName
            }
            ...
            Write-Log -Message "Failed to uninstall printer TSAPR-VS-M. Error: $($_.Exception.Message)" -Severity 3 -Source $deployAppScriptFriendlyName
            throw $_
        }
        finally {
            Stop-Transcript
